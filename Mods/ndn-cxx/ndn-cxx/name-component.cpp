...
parseUriEscapedValue(uint32_t type, std::string_view input)
{
  ...
  // std::cerr<< "value= "<<value.data()<<std::endl;
...
//Not so sure, Suppose the input shouldn't have anything like <type-number>=
  NDN_CXX_UNREACHABLE;
...
Component::fromEscapedString(std::string_view input)
{
  size_t equalPos = input.find('=');
  if (equalPos == std::string_view::npos) {
    // std::cerr<<"parseUriescaped "<<input<<std::endl;
