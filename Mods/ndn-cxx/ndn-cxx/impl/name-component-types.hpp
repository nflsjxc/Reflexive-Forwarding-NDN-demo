...
protected:
  ...
  {
    // std::cerr<<"writeUriEscaped called "<< comp.value()<<"\n";
...
class ReflexiveNameComponentType final: public ComponentType
{
public:
  ReflexiveNameComponentType(uint32_t type, std::string_view typeName, std::string_view uriPrefix)
    : m_type(type)
    , m_typeName(typeName)
...
ComponentTypeTable::ComponentTypeTable()
{
  ...
  static const ReflexiveNameComponentType ct59(tlv::ReflexiveNameComponent,"ReflexiveNameComponent"sv, "RN"sv); //typename uriprefix
  set(tlv::ReflexiveNameComponent, ct59);
}
